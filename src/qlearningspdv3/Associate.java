 ...
 * @author Wild
 */
final public class Associate extends JPanel{
     public double probability2;
     private Color color;
     private int x0;
     private int y0;
     private int agentsReward=0;
     ...
     public Associate(Stage startStage, Stage[] setOfStates,Color color){
     ...
     public void setTotalUtility2(int totalUtility2){
         this.totalUtility2=totalUtility2;
     }
    ...
    */
     ...
     public void moveRandomly(){
         ...
         agentsReward=setOfStates[i].getReward2();
         QLearningSPDv3.jtfReward1.setText(String.valueOf(agentsReward));
         System.out.println("Player's reward: "+agentsReward);
         totalUtility2=totalUtility2+agentsReward;
         QLearningSPDv3.jtfTotalUtilityPlayer.setText(String.valueOf(totalUtility2));
         repaint();
     }
     ...
     public void move(){
         ...
         * */
          for(int i=0;i<setOfStates.length;i++){
             if((tempCurrentStage.getX()==setOfStates[i].getX())&&(tempCurrentStage.getY()==setOfStates[i].getY()))
             {
                 tempCurrentStage=setOfStates[i];
                 System.out.println("Now an agent is in stage "+i);
                 break;
             }
         ...
         agentsReward=tempCurrentStage.getReward2();
         
         
         //then set the current state as agent's initial state
         setStartStage(tempCurrentStage);
         setCurrentStage(tempCurrentStage);
         QLearningSPDv3.jtfReward2.setText(String.valueOf(agentsReward));
         System.out.println("Associate's reward: "+agentsReward);
         totalUtility2=totalUtility2+agentsReward;
         QLearningSPDv3.jtfTotalUtilityAdversary.setText(String.valueOf(totalUtility2));
         repaint();
         }
         else
             repaint();
      ...
      */
      public void qLearn_v1(){
         ...
         if (!isStopped){
            ...
            System.out.println("Now prob is: "+probability2*10);
            if(n<=probability2*10)
            ...
            */
            ...
            //find max Q value
             ...
             //update Q value
             qValuesMatrix[currentPosition][currentAction]=(1-learningRate)*qValuesMatrix[currentPosition][currentAction]+learningRate*(nextStage.getReward2()+gamma*maxNextQ);
...
//            System.out.println("current Reward is "+tempCurrentStage.getReward2());
...
//            int index =associateStageWithAction(tempCurrentStage, selectedAction);
//            
...
//                tmp=searchArray(indexOfNeighbors,nextStageIndex);
//                if(tmp!=-1)
//                    nextStage=setOfNeighbors.get(tmp);//ArrayIndexOutOfBoundsException
//                else
...
//            //find current position
//            currentPosition=searchStateIndex(tempCurrentStage);
//            System.out.println("Associate's currentPosition "+currentPosition+ " size of setOfActions "+ setOfActions.size());
//            
//            
//            
//            System.out.println("Now agent is in stage : "+searchStateIndex(tempCurrentStage));
...
//            setOfStateIndices = findNeighboringStatesIndices(tempCurrentStage);
//            
//            for(i=0;i<setOfActions.size();i++)
//                System.out.println("Agent has an option to go: "+ setOfActions.get(i));
//            
//            System.out.println("IS 2 IN SETOFACTIONS?"+setOfActions.indexOf(23));
//            int [] setOfAllActions = {0,1,2,3};
//            //find the stage with maximum Q value
//            int maxQIndex=0;
//                double maxQ=qValuesMatrix[currentPosition][setOfAllActions[0]];
...
//                System.out.println();
//                System.out.println("Set of possible actions for state:"+setOfStateIndices.get(k));
//                for(int a=0;a<setOfNextActions.size();a++)
//                    System.out.println(setOfNextActions.get(a));
//            
//                System.out.println();
//                System.out.println("Set of possible states for state: "+setOfStateIndices.get(k));
...
//                //update Q value
//                qValuesMatrix[currentPosition][setOfActions.get(k)]=(1-learningRate)*qValuesMatrix[currentPosition][setOfActions.get(k)]+learningRate*(nextStage.getReward2()+gamma*max);
...
//                
//             /*
//             * In order to find a next state the agent searches the set of neighboring states indices and possible actions
//             * If he founds among q values with corresponding states and indices the maximum it moves to this position
//             * If there is no position with maximum q value (they all are the same) the agen choses the next state randomly
//             */
//            
//            //check, whether the neighboring q values equal to maximal q value  
//            
//            /*
//            boolean isQsEqual=false;
//            for(int k=0;k<setOfNeighbors.size();k++){
//                if(qValuesMatrix[currentPosition][setOfActions.get(k)]==maxQ && setOfActions.get(k)!=maxQIndex)
//                {
//                    isQsEqual=true;
//                    break;
//                }
...
//            
//            if(isQsEqual){
//                //selects a state from a set of states randomly
//                tempCurrentStage=searchStageRandomly(tempCurrentStage);
//                System.out.println("Agents takes an action "+findCurrentActionTaken(tempCurrentStage));
//            }
//            else{
...
//     
      ...
      public void qLearn_v4(){
         ...
         if (!isStopped){
           ...
           System.out.println("Player's reward: "+agentsReward);
            ...
            System.out.println("TOTAL UTILITY !!!!  "+totalUtility2);
     ...
     public int associateStageWithAction(Stage tempCurrentStage, int a){
         ...
         switch(a){
            ...
            if(!(((x>=100)&&(x<=200))&&((y>=0)&&(y<=350)))||(x==tempCurrentStage.getX()&& y==tempCurrentStage.getY()))
                index=-1;
            else{
                //walls
     ...
     ///////////////////////////////////////////////////////////////////////////
     /*
      * Relate a state to one of states from list of states
      */
     public Stage findState(Stage currentState){
         for(int i=0;i<setOfStates.length;i++){
                 if((currentState.getX()==setOfStates[i].getX())&&(currentState.getY()==setOfStates[i].getY()))
                 {
                     currentState=setOfStates[i];
                     //System.out.println("Now an agent is in stage "+i);
                     //System.out.println("i="+i+" X= "+setOfStates[i].getX()+" Y= "+setOfStates[i].getY());
                    // System.out.println("Previous stage was");
                    // System.out.println("X= "+startStage.getX()+" Y= "+startStage.getY());
                     break;
     ...
     public Stage searchStageRandomly(Stage tempCurrentStage){
         ...
         }while(!(((x>=100)&&(x<=200))&&((y>=0)&&(y<=350)))||(x==tempCurrentStage.getX()&& y==tempCurrentStage.getY()));
...
/*
      ...
      */
     /*
     public ArrayList[] qLearn(){
         int i;
         int j;
         int[] setOfActions = {0,1,2,3};
         int n=8;
         
         ArrayList[] qValuesMatrix = new ArrayList[8];
         for (int loop = 0; loop < n; loop++)
                    qValuesMatrix[loop] = new ArrayList();
         //initializing q matrix
         for(i=0;i<qValuesMatrix.length;i++){
             if(i%2==0){
                 for(j=0;j<2;j++)
                     qValuesMatrix[i].add(0);
             }
             else{
                 for(j=0;j<3;j++)
                     qValuesMatrix[i].add(0);
         ...
         //printing q matrix    
          for(i=0;i<qValuesMatrix.length;i++){   
             for(j=0;j<qValuesMatrix[i].size();j++)
                 System.out.print(qValuesMatrix[i].get(j)+" ");
             System.out.println();
         }
